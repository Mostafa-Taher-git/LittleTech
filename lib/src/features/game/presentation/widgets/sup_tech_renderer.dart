...
void drawSupTechRoundEye(Canvas canvas, SkinDefinition skin, double cx,
    double cy, double r, double s) {
  ...
  canvas.drawOval(
    Rect.fromCenter(
        center: center, width: (r + 4 * s) * 2, height: (r + 4 * s) * 2.5),
    Paint()
      ..shader = RadialGradient(colors: [
        skin.accentColor.withValues(alpha: 0.5),
        skin.accentColor.withValues(alpha: 0.0),
      ]).createShader(Rect.fromCenter(
          center: center, width: (r + 6 * s) * 2, height: (r + 6 * s) * 2.5)),
  );

  canvas.drawOval(
    Rect.fromCenter(
        center: center, width: (r + 1.5 * s) * 2, height: (r + 1.5 * s) * 2.5),
    Paint()
      ..shader = RadialGradient(colors: [
        ...
        skin.accentColor.withValues(alpha: 0.0),
          ...
          center: center, width: (r + 3 * s) * 2, height: (r + 3 * s) * 2.5)),
  );
...
void drawSupTechHappyEyes(Canvas canvas, SkinDefinition skin, double s,
...
void drawSupTechAngryEyes(Canvas canvas, SkinDefinition skin, double s,
...
void drawSupTechSurprisedEyes(Canvas canvas, SkinDefinition skin, double s,
    double eyeY, double eyeSpacing, double eyeR) {
  for (final dx in [-eyeSpacing, eyeSpacing]) {
...
void drawSupTechDeterminedEyes(Canvas canvas, SkinDefinition skin, double s,
    double eyeY, double eyeSpacing, double eyeR) {
  for (final dx in [-eyeSpacing, eyeSpacing]) {
    final center = Offset(dx, eyeY);
    canvas.drawOval(
...
void drawSupTechWinkEyes(Canvas canvas, SkinDefinition skin, double s,
    double eyeY, double eyeSpacing, double eyeR) {
  final dxLeft = -eyeSpacing;
  final path = Path()
    ..moveTo(dxLeft - 2.4 * s, eyeY + 0.1 * s)
    ..quadraticBezierTo(
        dxLeft, eyeY - 1.6 * s, dxLeft + 2.4 * s, eyeY + 0.1 * s);
  canvas.drawPath(
    path,
    Paint()
      ..color = skin.accentColor.withValues(alpha: 0.5)
...
void drawSupTechSleepEyes(Canvas canvas, SkinDefinition skin, double s,
    double eyeY, double eyeSpacing) {
  ...
  for (final dx in [-eyeSpacing, eyeSpacing]) {
    canvas.drawLine(
        Offset(dx - 2.5 * s, eyeY), Offset(dx + 2.5 * s, eyeY), linePaint);
  }
  final zPaint = Paint()
    ..color = skin.accentColor.withValues(alpha: 0.5)
    ..style = PaintingStyle.stroke
...
void drawSupTechErrorEyes(Canvas canvas, SkinDefinition skin, double s,
    double eyeY, double eyeSpacing, double eyeR) {
  final xPaint = Paint()
    ..color = skin.accentColor
    ..style = PaintingStyle.stroke
    ...
    ..strokeCap = StrokeCap.round;
  for (final dx in [-eyeSpacing, eyeSpacing]) {
    canvas.drawLine(Offset(dx - eyeR * 0.6, eyeY - eyeR * 0.6),
        Offset(dx + eyeR * 0.6, eyeY + eyeR * 0.6), xPaint);
    canvas.drawLine(Offset(dx + eyeR * 0.6, eyeY - eyeR * 0.6),
        Offset(dx - eyeR * 0.6, eyeY + eyeR * 0.6), xPaint);
  }
}
...
void drawSupTechFocusedEyes(
  ...
  final eyePaint = Paint()
    ...
    ..strokeWidth = 0.9 * s
    ..strokeCap = StrokeCap.round;
  canvas.drawLine(Offset(-eyeSpacing - 2 * s, eyeY - 0.5 * s),
      Offset(-eyeSpacing + 2 * s, eyeY + 0.5 * s), eyePaint);
  canvas.drawLine(Offset(eyeSpacing - 2 * s, eyeY - 0.5 * s),
      Offset(eyeSpacing + 2 * s, eyeY + 0.5 * s), eyePaint);
}
...
void _drawAntenna(
    Canvas canvas, SkinDefinition skin, double s, double hoodPeakY) {
...
void _drawWizardHat(
  ...
  canvas.drawRect(
      ...
      Paint()
        ...
        ..strokeWidth = 0.5 * s);
  canvas.drawCircle(
      Offset(0, peakY - 14 * s), 1.5 * s, Paint()..color = skin.accentColor);
}

void _drawHeadband(Canvas canvas, SkinDefinition skin, double s, double headCY,
    double headR, double hoodPeakY) {
  ...
  for (final dir in [-1, 1]) {
    ...
    final tailPath = Path()
      ...
      ..quadraticBezierTo(tailX + dir * 2 * s, bandY + 4 * s,
          tailX + dir * 1.5 * s, bandY + 7 * s)
      ..quadraticBezierTo(tailX + dir * 0.5 * s, bandY + 5 * s, tailX,
          bandY + bandH / 2 + 0.5 * s)
      ..close();
...
void _drawVisor(
    Canvas canvas, SkinDefinition skin, double s, double headCY, double headR) {
  ...
  final visorRect = RRect.fromRectAndRadius(
    Rect.fromCenter(
        center: Offset(0, visorY), width: headR * 1.6, height: headR * 0.7),
    Radius.circular(3 * s),
  );
  canvas.drawRRect(
      visorRect, Paint()..color = skin.accentColor.withValues(alpha: 0.35));
  canvas.drawRRect(
      visorRect,
      Paint()
        ..color = outlineColor
  ...
  canvas.drawRRect(
    RRect.fromRectAndRadius(
      Rect.fromCenter(
          center: Offset(0, visorY), width: headR * 1.2, height: headR * 0.35),
      Radius.circular(2 * s),
    ),
...
void _drawHorns(Canvas canvas, SkinDefinition skin, double s, double headCY,
    double headR, double hoodPeakY) {
  ...
  for (final dir in [-1, 1]) {
    ...
    final hornPath = Path()
      ...
      ..quadraticBezierTo(dir * headR * 1.0, hornBaseY - 6 * s,
          dir * headR * 0.7, hornBaseY - 11 * s)
      ..quadraticBezierTo(dir * headR * 0.55, hornBaseY - 9 * s,
          dir * headR * 0.45, hornBaseY - 1 * s)
      ..close();
    ...
    canvas.drawPath(
      Path()
        ...
        ..quadraticBezierTo(dir * headR * 0.75, hornBaseY - 7 * s,
            dir * headR * 0.6, hornBaseY - 10 * s),
      Paint()
...
void _drawHeadset(Canvas canvas, SkinDefinition skin, double s, double headCY,
    double headR, double hoodPeakY) {
  final isLightBody =
      ThemeData.estimateBrightnessForColor(skin.bodyColor) == Brightness.light;
  final accent = skin.accentColor;
  ...
  final bandPath = Path()
    ...
    ..cubicTo(-cupOffset, hoodPeakY - 6 * s, cupOffset, hoodPeakY - 6 * s,
        cupOffset, headCY);
  ...
  for (final dx in [-cupOffset, cupOffset]) {
    ...
    final innerRect = RRect.fromRectAndRadius(
      Rect.fromCenter(
          center: Offset(dx, headCY), width: cupW * 0.55, height: cupH * 0.65),
      Radius.circular(0.6 * s),
    );
    canvas.drawRRect(
        innerRect, Paint()..color = Colors.black.withValues(alpha: 0.2));
  }
  ...
  final micPath = Path()
    ...
    ..cubicTo(-cupOffset - 2 * s, headCY + 3 * s, -cupOffset - 1 * s,
        headCY + 5 * s, -cupOffset + 1 * s, headCY + 6 * s);
...
void _drawEarGlow(
  ...
  final isLight =
      ThemeData.estimateBrightnessForColor(skin.bodyColor) == Brightness.light;
  final outlineColor =
      isLight ? skin.accentColor.withValues(alpha: 0.6) : Colors.black87;
  final glowPaint = Paint();
...
void drawSupTechEarAccessory(
  ...
  switch (accessory) {
    ...
    case SupTechEarAccessory.headset:
      _drawHeadset(
          canvas, skin, s, headCY, headR, hoodPeakY ?? headCY - 20 * s);
...
void _drawBadge(Canvas canvas, SkinDefinition skin, double s, double bodyTopY,
    double bodyBotY) {
...
void _drawCape(Canvas canvas, SkinDefinition skin, double s, double bodyTopY,
    double bodyBotY) {
  final isLight =
      ThemeData.estimateBrightnessForColor(skin.bodyColor) == Brightness.light;
  final outlineColor = isLight ? const Color(0xFF94A3B8) : Colors.black87;
...
void _drawCodeScroll(Canvas canvas, SkinDefinition skin, double s,
...
void _drawGear(Canvas canvas, SkinDefinition skin, double s, double bodyTopY,
    double bodyBotY) {
  final gearY = bodyTopY + (bodyBotY - bodyTopY) * 0.45;
  final gearPaint = Paint()..color = skin.accentColor;
  canvas.drawCircle(
      Offset(0, gearY), 4 * s, gearPaint..style = PaintingStyle.fill);
  canvas.drawCircle(
      Offset(0, gearY),
      4 * s,
      Paint()
  ...
  for (var i = 0; i < 6; i++) {
    ...
    canvas.drawCircle(
        Offset(cx, cy + gearY), 1.5 * s, gearPaint..style = PaintingStyle.fill);
    canvas.drawCircle(
        Offset(cx, cy + gearY),
        1.5 * s,
        Paint()
...
void _drawFlameEmblem(Canvas canvas, SkinDefinition skin, double s,
    double bodyTopY, double bodyBotY) {
...
void _drawStaff(Canvas canvas, SkinDefinition skin, double s, double bodyTopY,
    double bodyBotY) {
  final staffX = 14 * s;
  ...
  canvas.drawOval(
    Rect.fromCenter(
        center: Offset(staffX, staffBot + 1.5 * s),
        width: 4 * s,
        height: 1.5 * s),
    Paint()
      ..shader = LinearGradient(
        ...
        colors: [
          skin.accentColor.withValues(alpha: 0.3),
          skin.accentColor.withValues(alpha: 0.0)
        ],
      ).createShader(Rect.fromCenter(
          center: Offset(staffX, staffBot + 1.5 * s),
  ...
  canvas.drawLine(
      Offset(staffX, staffTop),
      Offset(staffX, staffBot),
      Paint()
...
void drawSupTechPoseOverlay(
  ...
  switch (pose) {
      ...
      final glowPaint = Paint()
        ..shader = RadialGradient(colors: [
          ...
          skin.accentColor.withValues(alpha: 0.0),
            ...
            center: Offset(waveX, waveY), width: 10 * s, height: 10 * s));
    ...
    case SupTechPose.thinking:
      ...
      final glowPaint = Paint()
        ..shader = RadialGradient(colors: [
          ...
          skin.accentColor.withValues(alpha: 0.0),
            ...
            center: Offset(thoughtX, thoughtY), width: 8 * s, height: 8 * s));
      ...
      for (var i = 0; i < 3; i++) {
        canvas.drawCircle(
            Offset(thoughtX - 2.5 * s + i.toDouble() * 2.5 * s, thoughtY),
            0.8 * s,
            dotPaint);
      }
    case SupTechPose.working:
      ...
      final screenRect = RRect.fromRectAndRadius(
        Rect.fromCenter(
            center: Offset(0, screenY), width: 16 * s, height: 11 * s),
        Radius.circular(1.2 * s),
      );
      canvas.drawRRect(screenRect,
      ...
      canvas.drawOval(
        Rect.fromCenter(
            center: Offset(0, screenY + 7 * s), width: 12 * s, height: 3 * s),
        Paint()
          ..shader = LinearGradient(
            begin: Alignment.topCenter,
            end: Alignment.bottomCenter,
            colors: [
              skin.accentColor.withValues(alpha: 0.25),
              skin.accentColor.withValues(alpha: 0.0)
            ],
          ).createShader(Rect.fromCenter(
              center: Offset(0, screenY + 7 * s),
              width: 12 * s,
              height: 3 * s)),
      );
...
void drawNinjaFaceScarf(Canvas canvas, SkinDefinition skin, double s,
  ...
  final mask = Path()
    ...
    ..cubicTo(topEdge + 0.4 * s, maskTopY + 1.5 * s, midEdge + 0.5 * s,
        (maskTopY + maskBotY) / 2, botEdge + 0.2 * s, maskBotY - 0.5 * s)
    ..quadraticBezierTo(
        0, maskBotY + 0.6 * s, -botEdge - 0.2 * s, maskBotY - 0.5 * s)
    ..cubicTo(-midEdge - 0.5 * s, (maskTopY + maskBotY) / 2, -topEdge - 0.4 * s,
        maskTopY + 1.5 * s, -topEdge, maskTopY)
    ..close();
  ...
  canvas.drawPath(
    Path()
      ...
      ..quadraticBezierTo(
          0, maskTopY - 0.4 * s, topEdge - 0.6 * s, maskTopY + 0.1 * s),
    Paint()
      ..color = darker
      ..style = PaintingStyle.stroke
      ..strokeWidth = 0.6 * s
  ...
  // Fold 1 — upper cheek
  foldPaint
    ..color = darker.withValues(alpha: 0.28)
    ..strokeWidth = 0.4 * s;
  canvas.drawPath(
    Path()
      ..moveTo(-topEdge + 0.5 * s, maskTopY + 1.3 * s)
      ..quadraticBezierTo(
          0, maskTopY + 1.0 * s, topEdge - 0.5 * s, maskTopY + 1.3 * s),
    foldPaint,
  );

  // Fold 2 — mid cheek
  foldPaint
    ..color = darker2.withValues(alpha: 0.22)
    ..strokeWidth = 0.45 * s;
  canvas.drawPath(
    Path()
      ..moveTo(-midEdge + 0.2 * s, maskTopY + 2.4 * s)
      ..quadraticBezierTo(
          0, maskTopY + 2.1 * s, midEdge - 0.2 * s, maskTopY + 2.4 * s),
    foldPaint,
  );

  // Fold 3 — nose level
  foldPaint
    ..color = darker.withValues(alpha: 0.25)
    ..strokeWidth = 0.4 * s;
  canvas.drawPath(
    Path()
  ...
  canvas.drawPath(
    Path()
      ...
      ..quadraticBezierTo(
          0, maskTopY + 4.0 * s, botEdge - 0.3 * s, maskTopY + 4.3 * s),
    foldPaint,
  );
  ...
  canvas.drawRect(
    ...
    Paint()
      ..shader = LinearGradient(
        ...
        colors: [
          darker2.withValues(alpha: 0.15),
          darker2.withValues(alpha: 0.0)
        ],
